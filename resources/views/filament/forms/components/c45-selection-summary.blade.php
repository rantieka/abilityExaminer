...
@else
  <div style="background-color: white; border-radius: 0.75rem; padding: 1.5rem; color: #374151; box-shadow: 0 1px 2px 0 rgba(0, 0, 0, 0.05); border: 1px solid #e5e7eb; position: relative; overflow: hidden;">
    ...
    {{-- @if($c45Confidence <= $confidenceThreshold)
      <div style="margin-top: 1rem; background-color: #fffbeb; border: 1px solid #fde68a; padding: 0.75rem 1rem; border-radius: 0.5rem; color: #92400e; font-size: 0.8rem; display: flex; align-items: center; gap: 0.5rem; position: relative; z-index: 10;">
        ...
        <span><strong>Akurasi Rendah:</strong> Tingkat akurasi klasifikasi ({{ round($c45Confidence) }}%) di bawah batas target ({{ $confidenceThreshold }}%). Disarankan untuk melakukan peninjauan.</span>
      </div>
    @endif --}}
